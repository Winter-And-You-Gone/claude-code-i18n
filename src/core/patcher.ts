...
function getPostPatchRules(locale: string): PostPatchRule[] {
  ...
  if (baseLang === 'zh-CN') {
    return [
      ...
      // ── Remaining visible UI literals ──
      { search: '"Initializing…"', replace: '"启动中…"' },
      { search: '"Stopped"', replace: '"已停止"' },
      { search: '"Completed"', replace: '"已完成"' },
      { search: '"Idle"', replace: '"空闲"' },
      { search: '"Now using extra usage"', replace: '"已启用额外用量"' },
      { search: '"Editing notebook"', replace: '"编辑 notebook 中"' },
      { search: '"(No content)"', replace: '"（无内容）"' },
      { search: '"Fast mode"', replace: '"快速模式"' },
      { search: '"Free space"', replace: '"可用空间"' },
      { search: '"MCP tools"', replace: '"MCP 工具"' },
      {
        search: '"Note: You can already use backslash (\\\\\\\\) + return to add newlines."',
        replace: '"备注：你已经可以用反斜杠 (\\\\\\\\) + Enter 来换行。"',
      },
      { search: '"No model usage data available"', replace: '"没有可用的模型使用数据"' },
      { search: '"Tokens per Day"', replace: '"每日 Token 用量"' },
      { search: '"Remote Control"', replace: '"远程控制"' },
      { search: '"Sandbox:"', replace: '"沙箱："' },
      { search: '"Reconnecting"', replace: '"重新连接中"' },
      { search: '"Installation failed"', replace: '"安装失败"' },
      { search: '"Verification failed"', replace: '"验证失败"' },
      { search: '"No matches found"', replace: '"未找到匹配项"' },
      { search: '"Built-in agents"', replace: '"内置代理"' },
      { search: '"System prompt"', replace: '"系统提示"' },
      { search: '"not set"', replace: '"未设置"' },
      { search: '"Agents"', replace: '"代理"' },
      { search: '"Manage Claude Code plugins"', replace: '"管理插件"' },
      { search: '"Help improve Claude"', replace: '"帮忙改进 Claude"' },
      { search: '"go back"', replace: '"返回"' },
      { search: '"open in editor"', replace: '"在编辑器中打开"' },
      { search: '"Thinking"', replace: '"思索中…"' },
      { search: '"Back"', replace: '"返回"' },
      { search: '"Cancel"', replace: '"取消"' },
      { search: '"Continue"', replace: '"继续"' },
      { search: '"Exit"', replace: '"离开"' },
      { search: '"Enabled"', replace: '"已启用"' },
      { search: '"None"', replace: '"无"' },
      { search: '"Other"', replace: '"其他"' },
      { search: '"Authenticate"', replace: '"登录验证"' },
      { search: '"Reconnect"', replace: '"重新连接"' },
      { search: '"Uninstall"', replace: '"卸载"' },
      { search: '"Edit tools"', replace: '"编辑工具"' },
      { search: '"Inherit from parent"', replace: '"继承上层"' },
      { search: '"Theme"', replace: '"主题"' },
      { search: '"Language"', replace: '"语言"' },
      { search: '"Dark mode"', replace: '"深色模式"' },
      { search: '"Light mode"', replace: '"浅色模式"' },
      { search: '"Output style"', replace: '"输出风格"' },
      { search: '"Model"', replace: '"模型"' },
      { search: '"Proxy"', replace: '"代理服务器"' },
      { search: '"Version"', replace: '"版本"' },
      { search: '"Plugin agents"', replace: '"插件代理"' },
      { search: '"User settings"', replace: '"用户设置"' },
      { search: '"Remote Control connecting…"', replace: '"远程控制连接中…"' },
      { search: '"Remote Control reconnecting"', replace: '"远程控制重新连接"' },
      { search: '"Use auto mode during plan"', replace: '"计划期间使用自动模式"' },
      { search: '"Config"', replace: '"设置"' },
      { search: '"Mode"', replace: '"模式"' },
      { search: '"Overview"', replace: '"概览"' },
      { search: '"Current session"', replace: '"当前会话"' },
      { search: '"Fetch"', replace: '"获取"' },
      {
        search: '"This conversation is from a different directory."',
        replace: '"这个对话来自不同的目录。"',
      },
      {
        search: '"Sonnet 4.6 · Best for everyday tasks"',
        replace: '"Sonnet 4.6 · 日常任务最佳选择"',
      },
      { search: '"All tools"', replace: '"所有工具"' },
      { search: '"🪝 Hooks"', replace: '"🪝 钩子"' },
      {
        search: '`(\\$\\{[^}]+\\}) requires authentication\\. Check that the headersHelper script returns valid credentials, then use the \'Reconnect\' option\\.`',
        replace: '`$1 需要认证。请检查 headersHelper 脚本是否返回有效凭证，然后使用“重新连接”选项。`',
        regex: true,
      },
      {
        search: '`(\\$\\{[^}]+\\}) requires authentication\\. Use the \'Authenticate\' option\\.`',
        replace: '`$1 需要认证。请使用“登录验证”选项。`',
        regex: true,
      },
